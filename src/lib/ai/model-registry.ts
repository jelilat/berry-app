...
export const DEFAULT_MODEL_REGISTRY: BerryModelRegistry = {
  ...
  fast_reasoning: {
    provider: 'openai',
    model: process.env.BERRY_AI_FAST_MODEL?.trim() || 'gpt-5.6-luna',
    supportsTools: true,
    supportsStructuredOutput: true,
    temperature: 0.2,
  },
  strong_reasoning: {
    provider: 'openai',
    model: process.env.BERRY_AI_STRONG_MODEL?.trim() || 'gpt-5.6-sol',
    supportsTools: true,
    supportsStructuredOutput: true,
    temperature: 0.1,
  },
  code_reasoning: {
    provider: 'openai',
    model: process.env.BERRY_AI_CODE_MODEL?.trim() || 'gpt-5.6-sol',
    supportsTools: true,
    supportsStructuredOutput: true,
    temperature: 0,
  },
  fast_writer: {
    provider: 'openai',
    model: process.env.BERRY_AI_WRITER_MODEL?.trim() || 'gpt-5.6-luna',
    supportsTools: false,
    supportsStructuredOutput: true,
    temperature: 0.4,
  },
