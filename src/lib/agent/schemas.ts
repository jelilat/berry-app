...
const STUDIO_TOOL_CALL_SCHEMA = {
  ...
  properties: {
    ...
    board: {
      type: ["string", "null"],
      enum: ["esp32-devkit-v1", "arduino-uno", null],
    },
    componentType: {
      type: ["string", "null"],
      enum: [...COMPONENT_TYPES, null],
    },
    id: { type: ["string", "null"] },
    x: { type: ["number", "null"] },
    y: { type: ["number", "null"] },
...
export const ClarificationResultStructuredSchema: StructuredOutputSchema<ClarificationResult> =
  {
    ...
    description:
      "Whether the hardware build request is ready or needs clarification.",
    schema: {
      type: "object",
      additionalProperties: false,
      properties: {
...
export const AgentBuildPlanStructuredSchema: StructuredOutputSchema<AgentBuildPlan> =
  {
    ...
    description:
      "A safe hardware build plan that Berry can execute through tools.",
    schema: {
      type: "object",
      additionalProperties: false,
      properties: {
        ...
        components: {
          ...
          items: {
            ...
            properties: {
              type: {
                type: "string",
                enum: COMPONENT_TYPES as unknown as string[],
              },
              role: { type: "string" },
            },
            required: ["type", "role"],
        ...
        behavior: {
          ...
          properties: {
            kind: {
              type: "string",
              enum: ["blink", "serial_monitor", "custom"],
            },
...
export const AgentCircuitIntentStructuredSchema: StructuredOutputSchema<AgentCircuitIntent> =
  {
    ...
    description:
      "The bounded circuit implementation strategy selected by the circuit designer.",
    schema: {
      type: "object",
      additionalProperties: false,
      properties: {
...
export const WiringGuideDraftStructuredSchema: StructuredOutputSchema<WiringGuideDraft> =
  {
    ...
    description:
      "Human-readable wiring instructions based on the final Berry project graph.",
    schema: {
      type: "object",
      additionalProperties: false,
      properties: {
...
function validateClarificationResult(value: unknown): ClarificationResult {
  ...
  const questions = questionsRaw.map((question, index) => {
    ...
    return {
      ...
      question: requireString(
        q.question,
        `clarification.questions[${index}].question`,
      ),
      reason: requireString(
        q.reason,
        `clarification.questions[${index}].reason`,
      ),
  ...
  if (status === "needs_clarification") {
    if (questions.length === 0) {
      throw new Error(
        "clarification.questions must contain at least one question",
      );
    }
...
function validateAgentBuildPlan(value: unknown): AgentBuildPlan {
  ...
  const components = requireArray(object.components, "plan.components").map(
    (component, index) => {
  ...
  return {
    ...
    behavior: {
      kind,
      intervalMs:
        typeof behavior.intervalMs === "number"
          ? behavior.intervalMs
          : undefined,
      description: requireString(
        behavior.description,
        "plan.behavior.description",
      ),
    },
...
function validateAgentCircuitIntent(value: unknown): AgentCircuitIntent {
  ...
  const referenceCircuit = requireString(
    object.referenceCircuit,
    "circuitIntent.referenceCircuit",
  );
  if (
  ...
  return {
    ...
    toolCalls: requireArray(object.toolCalls, "circuitIntent.toolCalls").map(
      (call, index) =>
...
function validateStudioToolCall(value: unknown, path: string): StudioToolCall {
  ...
  if (tool === "studio.add_component") {
    return {
      ...
      rotationZ:
        typeof object.rotationZ === "number" ? object.rotationZ : undefined,
    };
  }
    ...
    return {
      ...
      color:
        typeof object.color === "string"
          ? requireWireColor(object.color, `${path}.color`)
          : undefined,
    };
  }
 ...
 * @param path Error path.
 */
function validateStudioEndpoint(
  value: unknown,
  path: string,
): StudioToolEndpoint {
  const object = requireRecord(value, path);
  ...
  if (breadboardId && !componentId && !terminalId) {
    return {
      breadboardId,
      site: parseBreadboardSite(object.site, `${path}.site`),
    };
  }
  throw new Error(
    `${path} must be either componentId+terminalId or breadboardId+site`,
  );
}
...
function requireWireColor(value: unknown, path: string): WireColor {
  if (
    typeof value !== "string" ||
    !(WIRE_COLORS as readonly string[]).includes(value)
  ) {
...
function optionalStringArray(value: unknown): string[] {
  return optionalArray(value).map((item, index) =>
    requireString(item, `array[${index}]`),
  );
}
